...
module.exports = function(Meteor) {
  const { Mongo, LocalCollection, underscore: _, EJSON } = Meteor;
  var advices = {}
  ...
  CollectionHooks.extendCollectionInstance = function extendCollectionInstance (self, constructor) {
    ...
    _.each(advices, function (advice, method) {
      var collection = self //Meteor.isClient || method === 'upsert' ? self : self._collection
...
// case this code changes.
  ...
  CollectionHooks.wrapCollection = function wrapCollection (ns, as) {
    ...
    ns.Collection = class Collection extends constructor {
      constructor(...args) {
        super(...args);
