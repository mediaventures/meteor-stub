module.exports = function(Meteor) {
  ...
  var Minimongo, MinimongoTest, MinimongoError;
  ...
  class LocalCollection {
    ...
    rawCollection() {
      ...
      return Object.assign({}, self, {
        findOneAndUpdate: function (query, modifier, options) {
          const itemToUpdate = self.findOne(query, options);
          if (itemToUpdate) {
            query = itemToUpdate;
          }
          self.update(query, modifier, options);
          return { value: itemToUpdate };
        }
      })
    }
    // XXX END KSINAS MOD
...
// the path.
  ...
  // XXX KSINAS MOD
  Meteor.Mongo = { Collection: LocalCollection };
